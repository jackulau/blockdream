...
# mineworld_wm -> blockdream_wm (dir + ~180 references + pyproject + the MINEWORLD_LOG env var).
...
if pgrep -f "mineworld_wm\.|blockdream_wm\." >/dev/null 2>&1; then
...
if [ -d src/mineworld_wm ]; then
  git mv src/mineworld_wm src/blockdream_wm 2>/dev/null || mv src/mineworld_wm src/blockdream_wm
fi
...
n=$(grep -rlZ --include='*.py' --include='*.toml' --include='*.md' --include='*.sh' \
  ...
  -e 'mineworld_wm' -e 'MINEWORLD_LOG' -e 'mineworld-wm' . 2>/dev/null \
  | tee /tmp/.rebrand_py_files | tr -dc '\0' | wc -c | tr -d ' ')
xargs -0 sed -i '' -e 's/mineworld_wm/blockdream_wm/g' -e 's/MINEWORLD_LOG/BLOCKDREAM_LOG/g' -e 's/mineworld-wm/blockdream-wm/g' < /tmp/.rebrand_py_files
