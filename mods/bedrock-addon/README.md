# mineworld Block-Art Player — Bedrock addon (Script API)
...
## Status
**Code complete; operator-import.** Not runnable in the mineworld CI sandbox
(no Bedrock client). To use:

1. Generate the pack from any GIF/video:
   ```
   mineworld render clip.gif --target bedrock-script --grid 64x64 --out my_addon/
   ```
...
- `behavior_pack/scripts/main.js` — `system.runInterval` loop applying per-frame
  ...
  `@mineworld/emit-commands` `BEDROCK_PLAYER_JS` (the generator's source of truth).
...
> `generateBedrockScriptAddon` in `@mineworld/emit-commands`.
