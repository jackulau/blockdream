/**
 * Minimal Java-edition NBT (Named Binary Tag) writer + reader.
 *
 * Java NBT is BIG-ENDIAN and (for files like map_<n>.dat / level.dat) gzip-compressed.
 * This implements the tag subset needed by the map + structure emitters:
 * End, Byte, Short, Int, Long, Float, Double, ByteArray, String, List, Compound,
 * IntArray, LongArray.
 *
 * Bedrock NBT is little-endian — see @mineworld/emit-bedrock for that variant.
 */
...
// Tagged value wrappers so a JS object can express NBT's typed scalars unambiguously.
...
class ByteWriter {
  ...
  i8(v: number): void {
    // NBT bytes are signed; store the low 8 bits (reads back via readInt8).
    this.push(Buffer.from([v & 0xff]));
  }
  i16(v: number): void {
...
class ByteReader {
  constructor(private buf: Buffer, private pos = 0) {}
  u8(): number {
